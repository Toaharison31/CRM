...
public class UtilisateurDAO { 
    ...
    /**
     * Fomba pro hikilasiana ny teny miafina ho SHA-256 (Hachage mivantana eto
     * amin'ny DAO)
     */
    ...
    /**
     * Remplit les paramètres d'un PreparedStatement avec les données d'un
     * utilisateur.
     */
    ...
    public Utilisateur seConnecter(String email, String mdp) {
        String sql = "SELECT id, nom, email, mot_de_passe, role, cree_le FROM utilisateur WHERE email = ?";
        ...
        try (Connection conn = Connexion.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    if (hashSaisi.equals(u.getMot_de_passe())) {
                        System.out.println("Authentification réussie pour: " + email);
                        java.awt.EventQueue.invokeLater(new Runnable() {
                            public void run() {
                            try { 
                                ui.DashboardFrame dashboard = new ui.DashboardFrame();
